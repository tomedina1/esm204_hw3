...
mec_final %>% 
  kable(col.names = c('SCC ($/lb)', 'Welfare High Consumer ($)', 'Welfare Low Consumer ($)', 'Producer Welfare ($)')) %>% 
  kable_styling(bootstrap_options = c('hover', 'striped'))
```

#6
```{r}
# demand after tax and cut in half
demand_solar <- function(price, model) {
  q <- (price - (0.5*model$coefficients[[1]])) / model$coefficients[[2]]
  q <- ifelse(q < 0, 0, q)
  return(q)
}
...
demand_agg_solar <- function(price, tax) {
  new_price <- price + tax
  q <- demand_solar(new_price, low.model) + demand(price, high.model)
  return(q)
}
...
lm.demand_agg_solar <- lm(price ~ ., data = solar_aggd.lm)
